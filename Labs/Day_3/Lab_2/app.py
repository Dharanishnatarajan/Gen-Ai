...
def analyze_sentiment(state: WorkflowState) -> WorkflowState:
    ...
    if not query:
        ...
        try:
            ...
            score = result["score"]
            # Route only when the classifier is clearly confident.
            if score < 0.72:
                route = "NEUTRAL"
            elif label == "POSITIVE":
                route = "POSITIVE"
...
def positive_handler(state: WorkflowState) -> WorkflowState:
    prompt = "You are a warm conversation coach. The user's message landed in the POSITIVE branch. Summarize the emotional signal you noticed, then suggest an upbeat reply in 2 or 3 sentences."
    return generate_response(state, prompt, "positive_handler")

def negative_handler(state: WorkflowState) -> WorkflowState:
    prompt = "You are a calm support coach. The user's message landed in the NEGATIVE branch. Briefly explain the emotional signal you noticed, then suggest a kind, de-escalating reply in 2 or 3 sentences."
    return generate_response(state, prompt, "negative_handler")

def neutral_handler(state: WorkflowState) -> WorkflowState:
    prompt = "You are a practical chat assistant. The user's message landed in the NEUTRAL branch. Describe the likely intent, then suggest a clear next reply in 2 or 3 sentences."
